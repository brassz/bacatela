...
let hasCidadesIdsColumn = null;

function cidadesIdsColumnMissing(e) {
  return /cidades_ids|42703/i.test(String(e && e.message || ''));
}

function hasCidadesIds() {
  return hasCidadesIdsColumn === true;
}
...
async function writeUsers(arr) {
  ...
  try {
    ...
    if (!cidadesIdsColumnMissing(e)) throw e;
    hasCidadesIdsColumn = false;
    await rest('POST', '/rest/v1/gestaoemprestimosalex_users?on_conflict=id', rows.map(({ cidades_ids, ...r }) => r), {
      Prefer: 'resolution=merge-duplicates,return=minimal'
    });
  }
}
...
async function updateUser(id, patch) {
  ...
  try {
    ...
    if (!cidadesIdsColumnMissing(e) || row.cidades_ids === undefined) throw e;
    hasCidadesIdsColumn = false;
    const { cidades_ids, ...semCidades } = row;
    if (!Object.keys(semCidades).length) {
      const err = new Error('Coluna cidades_ids ausente no Supabase. Execute supabase/migrate_user_cidades.sql no SQL Editor.');
      err.code = 'cidades_ids_missing';
      throw err;
    }
    const rows = await rest(
      'PATCH',
...
async function pingSchema() {
  ...
  await rest('GET', '/rest/v1/gestaoemprestimosalex_users?select=id&limit=1');
  try {
    await rest('GET', '/rest/v1/gestaoemprestimosalex_users?select=id,cidades_ids&limit=1');
    hasCidadesIdsColumn = true;
  } catch (e) {
    if (!cidadesIdsColumnMissing(e)) throw e;
    hasCidadesIdsColumn = false;
    console.warn('Supabase: coluna cidades_ids ausente. O app funciona, mas cidades por usuário exigem supabase/migrate_user_cidades.sql.');
  }
}
...
function envStatus() {
  return {
    ...
    temCidadesIds: hasCidadesIdsColumn === true,

...
async function readDbRest() {
  const [metaRows, clientes, cidades, emprestimos, parcelas, pagamentos] = await Promise.all([
...
async function readDb() {
  try {
    ...
    if (!x || !x.data) return { revision: 0, data: { clientes: [], emprestimos: [], pagamentos: [], cidades: [] } };
...
/** Garante o admin ALEX (senha ALEX123) mesmo se a base já tiver usuários. */
async function ensureAlexAdmin(hashSenha) {
  const arr = await readUsers();
  if (arr.some(u => String(u.usuario || '').toLowerCase() === 'alex')) return;
  const cred = hashSenha(process.env.ALEX_PASSWORD || 'ALEX123');
  arr.push({
    id: crypto.randomUUID(),
    usuario: 'alex',
